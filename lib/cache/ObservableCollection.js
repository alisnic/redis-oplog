...
export default class ObservableCollection {
  ...
  async setupCollection() {
    ...
    this._projectionFn = fields ? LocalCollection._compileProjection(projection) : (doc) => doc;

    this._sharedProjection = this.matcher.combineIntoProjection(projection);

    if (!fields && !this.matcher) {
      // if we have no fields and no matcher, we can just save the doc to store as it is.
      this._sharedProjectionFn = (doc) => doc;
    } else {
      if (this.sorter) {
